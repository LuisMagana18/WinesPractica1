...
import com.mgoc.winespractica1.databinding.ActivityLoginBinding
...
class LoginActivity : AppCompatActivity() {
    ...
    private fun validaCampos(): Boolean {
        ...
        if(email.isEmpty()){
            binding.tietEmail.error = "Ingrese un correo valido"
            binding.tietEmail.requestFocus()
            return false
        }

        if(contrasenia.isEmpty() || contrasenia.length < 6){
            binding.tietContrasenia.error = "Ingrese una contraseña o la contraseña no tiene por lo menos 6 caracteres"
